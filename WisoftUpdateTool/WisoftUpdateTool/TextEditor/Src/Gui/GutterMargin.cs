...
namespace ICSharpCode.TextEditor
{
	...
	public class GutterMargin : AbstractMargin, IDisposable
	{
		...
		static GutterMargin()
		{
			Stream cursorStream = Assembly.GetCallingAssembly().GetManifestResourceStream("WisoftUpdateTool.Resources.RightArrow.cur");
		...
		public int DefaultGutterMarginLenth
		{
			get
			{
				return 4;
		...
		public override Size Size {
			get {
				return new Size((int)(textArea.TextView.WideSpaceWidth
									  * Math.Max(DefaultGutterMarginLenth, (int)Math.Log10(textArea.Document.TotalNumberOfLines) + 1)),
				                -1);
			}
		}
		...
		public override void Paint(Graphics g, Rectangle rect)
		{
			...
			for (int y = 0; y < (DrawingPosition.Height + textArea.TextView.VisibleLineDrawingRemainder) / fontHeight + 1; ++y) {
				...
				Rectangle backgroundRectangle = new Rectangle(drawingPosition.X, ypos - 1, drawingPosition.Width, fontHeight);
				if (rect.IntersectsWith(backgroundRectangle)) {
					...
					if (curLine < textArea.Document.TotalNumberOfLines) {
						g.DrawString((curLine + 1).ToString().PadLeft(Math.Max(DefaultGutterMarginLenth, (int)Math.Log10(textArea.Document.TotalNumberOfLines) + 1), ' '),
